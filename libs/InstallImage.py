...
class SyslinuxCfg(object):
    ...
    def setMessage(self, message_color, message):
        ...
        if len(message_color) != 2:
            raise ValueError(_("message_color string must be 2 bytes long.  Passed in string was: %s bytes long.  String: %s") % (len(message_color), message(color)))
...
class InstallImage(object):
    ...
    def __init__(self, project, target, name, progress_callback = None):
        ...
        if (not self.kernels) and (not self.default_kernel):
                raise ValueError(_("no kernels were found"))
        self.kernels.sort()
...
class BaseUsbImage(InstallImage):
    ...
    def umount_container(self):
        if self.tmp_path:
            ...
            if not result:
                print >> sys.stderr, _("Error unmounting: %s") % self.tmp_path
                raise EnvironmentError, _("Error unmounting: %s") % self.tmp_path
...
class HddImage(InstallImage):
    def create_image(self):
        raise ValueError(_("HddImage: Create Hard Disk Image not implemented!"))
...
if __name__ == '__main__':
    cnt = len(sys.argv)
    if (cnt != 4) and (cnt != 2):
        print >> sys.stderr, _("USAGE: %s proj_path proj_name platform_name") % (sys.argv[0])
        print >> sys.stderr, _("       %s proj_name") % (sys.argv[0])
        sys.exit(1)
